...
// Attempt to restore admin.html from a likely intact version or backup
// If scratch/admin_master.html exists, it might be the best source.
let backupPath = 'scratch/admin_master.html';
if (!fs.existsSync(backupPath)) {
    // Try the "unzipped_old" if it exists
    backupPath = 'scratch/unzipped_old/admin.html';
}

if (fs.existsSync(backupPath)) {
    console.log(`📡 Restoring admin.html from ${backupPath}`);
    let content = fs.readFileSync(backupPath, 'utf8');
    
    // Perform the Lifts cleanup again ON THE FRESH CONTENT
    content = content.replace(/<button class="btn btn-info" onclick="window\.location\.href='lifts\.html'"[\s\S]*?<\/button>/, '');
    
    // Add the chat button carefully
    ...
    // Add the chat widget and JS at the ABSOLUTE end of the body
    const chatWidget = `
    <!-- Admin Chat Widget -->
...
`;
    ...
    console.log('✅ admin.html RESTORED AND UPDATED.');
} else {
