...
// 3. إضافة مسار الاستيراد بـ exceljs
const salaryRoute = `
...
// البحث عن مكان الإدراج بعد مسار حذف الموظف
const insertionPoint = 'res.json({ message: "تم حذف الموظف وجميع بياناته بنجاح" });';
const pos = content.indexOf(insertionPoint);

if (pos !== -1) {
    const endOfRoute = content.indexOf('});', pos) + 3;
    content = content.substring(0, endOfRoute) + "\n" + salaryRoute + content.substring(endOfRoute);
    fs.writeFileSync('server.js', content, 'utf8');
    console.log("✅ تم بناء السيرفر بنجاح!");
} else {
    console.log("❌ فشل تحديد نقطة الإدراج.");
}
