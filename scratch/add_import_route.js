...
// أضف المسار الجديد بعد مسار حذف الموظف (DELETE /api/employees/:id)
const insertAfter = "res.json({ message: \"تم حذف الموظف وجميع بياناته بنجاح\" });";
...
// أوجد نهاية الـ route (الـ });  بعد هذه الجملة)
const afterInsert = content.indexOf('});', insertPoint);
const endOfRoute = afterInsert + 3; // نهاية `});`

content = content.substring(0, endOfRoute) + newRoute + content.substring(endOfRoute);
fs.writeFileSync('server.js', content, 'utf8');
console.log("✅ تمت إضافة مسار استيراد الرواتب بنجاح");

// التحقق من صحة الكود
const { execSync } = require('child_process');
try {
    execSync('node --check server.js', { stdio: 'pipe' });
    console.log("🎉 الملف صحيح تماماً!");
} catch(e) {
    console.log("❌ خطأ في الملف:", e.stderr?.toString());
}
